...
<div class="container py-3" style="color: #212529; font-family: system-ui, -apple-system, sans-serif;">
    ...
    <!-- МЕНЮ НАВИГАЦИИ АДМИНИСТРАТОРА (ЗАКАЗЫ / ТОВАРЫ / ПОЛЬЗОВАТЕЛИ) -->
    ...
    @else
        <!-- ДЕСКТОПНАЯ СТРОГАЯ ТАБЛИЦА (ЭКРАНЫ >= 768px) -->
                    ...
                    @foreach($orders as $order)
                        <tr class="border-bottom border-subtle-gray">
                            ...
                            <td class="py-3 text-center border-subtle-gray">
                                @if($order->status === 'new')
                                    <span class="badge bg-light text-dark border border-dark-subtle px-2 py-1 text-uppercase font-monospace w-100" style="font-size: 0.7rem;">ПОЛУЧЕН</span>
                                @elseif($order->status === 'manufacturing')
                                    <span class="badge bg-success-subtle px-2 py-1 text-uppercase font-monospace w-100" style="color: #198754; border: 1px solid #198754; font-size: 0.7rem;">В ЦЕХУ</span>
                                @elseif($order->status === 'delivery')
                                    <span class="badge bg-info-subtle text-info px-2 py-1 text-uppercase font-monospace w-100" style="border: 1px solid #0dcaf0; font-size: 0.7rem;">В ПУТИ</span>
                                @elseif($order->status === 'completed')
                                    <span class="badge bg-dark text-white px-2 py-1 text-uppercase font-monospace w-100" style="font-size: 0.7rem;">ВЫДАН</span>
                                @else
                                    <span class="badge bg-secondary text-white px-2 py-1 text-uppercase font-monospace w-100" style="font-size: 0.7rem;">{{ $order->status }}</span>
                                @endif
                            </td>
        ...
        <div class="d-md-none d-flex flex-column gap-3">
            @foreach($orders as $order)
                <div class="border border-subtle-gray p-3 bg-white">
                    ...
                    <div class="mb-3">
                        @if($order->status === 'new')
                            <span class="badge bg-light text-dark border border-dark-subtle px-2 py-1 text-uppercase font-monospace d-block text-center" style="font-size: 0.75rem;">ПОЛУЧЕН</span>
                        @elseif($order->status === 'manufacturing')
                            <span class="badge bg-success-subtle px-2 py-1 text-uppercase font-monospace d-block text-center" style="color: #198754; border: 1px solid #198754; font-size: 0.75rem;">В ЦЕХУ</span>
                        @elseif($order->status === 'delivery')
                            <span class="badge bg-info-subtle text-info px-2 py-1 text-uppercase font-monospace d-block text-center" style="border: 1px solid #0dcaf0; font-size: 0.75rem;">В ПУТИ</span>
                        @elseif($order->status === 'completed')
                            <span class="badge bg-dark text-white px-2 py-1 text-uppercase font-monospace d-block text-center" style="font-size: 0.75rem;">ВЫДАН</span>
                        @else
                            <span class="badge bg-secondary text-white px-2 py-1 text-uppercase font-monospace d-block text-center" style="font-size: 0.75rem;">{{ $order->status }}</span>
                        @endif
                    </div>
                    ...
                    <!-- Форма обновления статуса с крупными элементами клика (>= 44px) -->
